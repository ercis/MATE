"""Performance over time - how the performance KPIs evolve.

Slices the event log along time (whole cases grouped by start time), runs the
vendored performance KPIs (:mod:`perf_core`) on each slice, and returns a
...
def _cache_key(
    ...
    *,
    ...
    if mode == "absolute":
        return f"ts_absolute_n{slices}_m{min_cases}"
    if mode == "sliding":
        return f"ts_sliding_w{window}_s{step}_m{min_cases}"
    return f"ts_calendar_{granularity}_m{min_cases}"
...
class PerformanceOverTimeModule(Module):
    id = "performance_over_time"

    guidance_system_prompt = (
        "You are a process-mining analyst interpreting how performance metrics "
        "evolve over the lifetime of a process. Each point is a time slice - "
        "whole cases grouped by their start time. Read the series for trends, "
        "spikes, and regime shifts in cycle time (avg / median / p90 / p95), "
        "throughput, and lead time. Reference specific slice labels and values, "
        "and distinguish genuine drift from thin slices (low n_cases). Suggest "
    ...
    async def guidance_payload(self, ctx: ModuleContext) -> dict[str, Any] | None:
        ...
        # without every metric per slice.
        headline = (
            "avg_cycle_time_s",
            "median_cycle_time_s",
            "p90_cycle_time_s",
    ...
    async def timeseries(
        ...
        if mode == "absolute":
            ...
            key = _cache_key("absolute", min_cases=min_cases, slices=n)
        elif mode == "sliding":
            params = {"window": float(window), "step": float(step)}
            key = _cache_key("sliding", min_cases=min_cases, window=float(window), step=float(step))
        else:
            params = {"granularity": granularity}
            key = _cache_key("calendar", min_cases=min_cases, granularity=granularity)
    ...
    @job(progress=True, title="Performance over time - precompute")
