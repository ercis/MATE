...
import {
  usePerformanceOverTimeTimeseries,
  useDriftPeriods,
  type DriftPeriod,
  type PerfSliceMetrics,
  type SliceMode,
  type SlicePoint,
  type TimeseriesParams,
} from "./queries";

// Friendly labels reused from the performance panel's KPI labels.
...
const DEFAULT_KPI = "avg_cycle_time_s";
...
// human-readable durations (s / min / h / d …) rather than raw second counts.
function isDurationKey(key: string): boolean {
  return key.endsWith("_s");
}

// Format one metric value for its key: durations via formatDuration, everything
// else as a plain number (integers verbatim, floats to 3 dp).
...
export function PerformanceOverTimePanel({ logId }: { logId: string; moduleId: string }) {
  ...
  // Min-max each series to [0,1] so metrics on different scales (e.g. throughput
  // vs. cycle time) stay comparable on one shared axis.
  ...
  const q = usePerformanceOverTimeTimeseries(logId, mode, params);
  ...
  const chartData = useMemo<ChartDatum[]>(() => {
    ...
    for (const key of selectedKpis) {
      ...
      let max = -Infinity;
      for (const s of pts) {
        const v = s.metrics?.[key as keyof PerfSliceMetrics];
    ...
    <div className="space-y-6">
      <header className="space-y-1">
        ...
        <p className="text-xs text-muted-foreground">
          Whole cases grouped by start time, sliced along the timeline. Cycle
          times &amp; throughput per slice.
        </p>
      </header>
...
function ChartBody({
    ...
    return (
      <Card>
        <CardContent className="text-sm text-muted-foreground">
          Could not load the performance time series. Re-import the log or check
          the module&apos;s logs.
        </CardContent>
      </Card>
    );
  ...
  // Format the Y-axis as a duration only when every plotted line is a duration
  // and the axis carries real values (i.e. not the unitless 0–1 normalised one).
        ...
        <ResponsiveContainer width="100%" height={400}>
          <LineChart data={data} margin={{ top: 10, right: 16, left: 0, bottom: 16 }}>
            ...
            <YAxis
              ...
              tickFormatter={
                durationAxis ? (v: number) => formatDuration(v) : undefined
              }
              allowDecimals
            />
            <Tooltip
