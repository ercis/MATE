"""Time slicing for the *Performance over time* module.
...
stay intact and the performance KPIs in :mod:`perf_core` remain valid on each
per-slice sub-log.
...
# Vendored from ``modules/performance`` (via :mod:`perf_core`) so this module
# stays self-contained - see modules/performance_over_time/manifest.yaml.
from .perf_core import compute_perf_metrics

# Hard cap so a tiny step / huge N can't spin up an unbounded number of
# slices (and an unbounded number of performance runs).
_MAX_SLICES = 2000
...
def _numeric_metric_keys(metrics: dict[str, Any]) -> list[str]:
    """Numeric (or nullable-numeric) metric keys.

    ``compute_perf_metrics`` always returns the full key set with consistent
...
def compute_timeseries(
    ...
    *,
    ...
    """Build time slices and run the performance KPIs on each sub-log.

    Returns the ``performance_timeseries`` result schema: ``metric_keys`` (the
    ...
    for i, spec in enumerate(specs):
        ...
        if n_cases >= min_cases:
            ...
            metrics = compute_perf_metrics(sub_df) or None
            n_events = len(sub_df)
