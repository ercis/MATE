...
Source:
https://github.com/yesanton/Process-Drift-Visualization-With-Declare/blob/master/src/data_algorithms_cluster_and_change_point.py
Author: Anton Yeshchenko
Note: Adapted for the purposes of this work.
'''
...
def remove_redundant_timeseries_HEADER(data_uncut):
    ...
    print('there are : ' + str(len(data)) +
          " values left after deleting 100, and 0s")
    print("there were: " + str(zeros_removed) + " vectors with zeros removed")
    print("there were: " + str(hundreds_removed) +
          " vectors with hundreds removed")
    print("there were: " + str(nonchaning_removed) +
          " vectors with non changing values removed")
...
def do_cluster_changePoint(data_uncut, 
                           linkage_method="ward", 
    ...
    Z = linkage(data_cut, method=linkage_method,
                metric=linkage_metric)  # metric='correlation'
    a = fcluster(Z, cutoff, fcluster_metric)

    d = dict()
    for cluster_n, data in zip(a, data_uncut):
        if cluster_n not in d:
            d[cluster_n] = [data]
        else:
    ...
    x_all_lines = {}
    # in ths case we want to detect the drifts in the whole 
    # range of constrains at the same time
    c = rpt.costs.CostRbf()
    ...
    if cp_all:
        ...
        for dk in order_cluster:
            ...
            sig = np.array(dd)
            signal = np.transpose(sig)
            algo = rpt.Pelt(model="rbf", custom_cost=c,
                            min_size=2, jump=2).fit(signal)
...
def calculate_erratic_value(power_smooth, xnew, averaged_line):
    ...
    ynew = [(power_smooth[0] + power_smooth[1] + power_smooth[2]) / 3] + \
           [(power_smooth[0] + power_smooth[1] + power_smooth[2] + \
               power_smooth[3]) / 4] + [(i + j + k + l_ + g) / 5 for i, j, k, l_, g in
            zip(power_smooth[:-4], power_smooth[1:-3], power_smooth[2:-2], \
                power_smooth[3:-1], power_smooth[4:])] + \
           [(power_smooth[-4] + power_smooth[-3] + power_smooth[-2] + \
               power_smooth[-1]) / 4] + \
    ...
    for (y, y_original) in zip(ynew[1:], power_smooth[1:]):
        ...
        # (length of the curve)
        real_erratic_score += np.sqrt((xnew[1] - xnew[0])
                                      ** 2 + ((y - y_0) * len(averaged_line)) ** 2)

        # get the erratic score as in the paper (polyline)
        # polyline_erratic_score += numpy.math.sqrt((xnew[1] - xnew[0]) ** 2 + (
            # (y_original - y_0_polyline) * len(averaged_line)) ** 2)
        polyline_erratic_score += np.sqrt(
            1 + ((y_original - y_0_polyline) * len(averaged_line)) ** 2)
