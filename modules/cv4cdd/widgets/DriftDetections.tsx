...
import { useCv4cddResults } from "../panel/queries";

/** Computer-vision-detected concept drifts (CV4CDD) with type + confidence. */
export default function DriftDetections({
  logId,
  config,
}: {
  logId: string;
  ...
  const { data, isLoading, isError } = useCv4cddResults(logId);
  ...
  if (!data?.ran) {
    return (
      <div className="flex h-full min-h-24 items-center justify-center px-3 text-center text-xs text-muted-foreground">
        Run detection in the CV4CDD module to populate this card.
      </div>
    );
  }
    ...
    <div className="flex h-full flex-col">
      <div className="mb-2 flex items-baseline gap-2">
        ...
        <span className="text-xs text-muted-foreground">
          detection{drifts.length === 1 ? "" : "s"} across {data.n_windows} windows
        </span>
      </div>
