"""Time slicing for the *Complexity v2 over time* module.
...
stay intact and the entropy / distance math in :mod:`metrics_core` remains
valid on each per-slice sub-log.

The windowing is identical to ``modules/complexity_over_time`` (same three
modes, same case-start bucketing); only the per-slice metric bundle differs -
here every slice is scored with the full Langer Table 3.3 suite
(:func:`metrics_core.compute_all`) instead of the vendored EPA-only core.
...
# Vendored from ``modules/complexity_v2`` so this module stays self-contained -
# see modules/complexity_v2_over_time/manifest.yaml and the module plan.
from .metrics_core import METRIC_DEFS, compute_all
...
# Paper-ordered metric keys (Table 3.3) - the Y-axis dropdown source. Restricting
# to METRIC_DEFS keeps compute_all's run metadata (df_edges, *_log10, the
# enriched/downsampled flags) out of the selector.
_ALL_METRIC_KEYS: list[str] = [d["key"] for d in METRIC_DEFS]
...
def _numeric_metric_keys(metrics: dict[str, Any]) -> list[str]:
    """Numeric (or nullable-numeric) Table 3.3 metric keys, in paper order.

    ``compute_all`` always returns the full key set with consistent keys (some
    values may be ``None`` for tiny slices or an absent enriched-attribute
    schema), so the keys from any one populated slice describe the whole series.
    """
    keys: list[str] = []
...
def compute_timeseries(
    ...
    *,
    ...
    """Build time slices and run the full v2 complexity suite on each sub-log.

    Returns the ``complexity_v2_timeseries`` result schema: ``metric_keys`` (the
    dropdown source) plus one point per slice carrying all metrics, so the panel
    can switch the Y-axis metric without refetching.

    ``detected_schema`` (optional) enables the enriched-entropy measures per
    slice when the IEEE-XES attribute set is present; ``max_variants`` caps the
    Levenshtein-based distance metrics exactly as in the base complexity_v2
    module. ``progress`` (optional) receives ``(fraction, message)`` once per
    computed slice so long series stay observable in the jobs UI.
    """
    ...
    for i, spec in enumerate(specs):
        ...
        if n_cases >= min_cases:
            ...
            metrics = compute_all(
                sub_df, detected_schema=detected_schema, max_variants=max_variants
            )
    ...
    if metric_keys is None:
        ...
        full = compute_all(df, detected_schema=detected_schema, max_variants=max_variants)
        metric_keys = _numeric_metric_keys(full) if full and not full.get("empty") else []
